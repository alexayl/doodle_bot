...
class BLEPacketHandler:
    ...
    def _handle_rx_with_timing(self, _, data: bytearray):
        ...
        self.transmission_times.append((time.time() - self.sent_time))
        ...
        while True:
            try:
                ...
                if self.last_received_packet_id == packet_id and self.last_received_message == expected_response:
                    ...
                    self.response_received = False
                    print(self.transmission_times)
                    print(f"Request time: {self.transmission_times[0]*2000:.3f} ms")
    ...
    async def send_packets(self, all_commands):
        ...
        print(f"Average time to enqueue signals: {sum(self.transmission_times) / len(self.transmission_times)*1000:.3f} ms")